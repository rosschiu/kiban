...
// newTestPool connects to the live Postgres as kiban_docs (the runtime role — migrations must
// already be applied via `make migrate-docs`).
func newTestPool(t *testing.T) *pgxpool.Pool {
	return livestack.Pool(t, "kiban_docs", testEnv(t, "KIBAN_DOCS_DB_PASSWORD"))
}

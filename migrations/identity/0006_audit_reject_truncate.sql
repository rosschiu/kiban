...
-- BEFORE TRUNCATE statement trigger on the identity audit table — the row-level
-- append-only trigger (0004) never fires for TRUNCATE. Same pattern as
...
-- here only keeps this tree self-contained (sqlc parses migrations/identity alone) and
...
CREATE TRIGGER identity__events_reject_truncate
    BEFORE TRUNCATE ON audit.identity__events
    FOR EACH STATEMENT EXECUTE FUNCTION audit.reject_mutation();

---- create above / drop below ----

DROP TRIGGER identity__events_reject_truncate ON audit.identity__events;
